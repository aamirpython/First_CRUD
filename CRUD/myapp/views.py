...
def form_data(request):
    if request.method == 'POST':
        ...
        elif Player.objects.filter(Email_name=Email_name).exists():
...
def Login_form(request):
            if request.method == 'POST':
                ...
                if Player.objects.filter(phone_number=phone_number).exists():
                    obj = Player.objects.get(phone_number=phone_number)
...
def data(request):
    persons = Player.objects.filter(is_active=True).order_by('id')
...
def delete_user(request):
    if request.method == 'POST':
        ...
        uid = json.loads(data)
        if Player.objects.filter(id=uid).exists():
            Player.objects.filter(id=uid).update(is_active=False)
...
def update_form_data(request):
    if request.method == 'POST':
        ...
        Player.objects.filter(id=uid).update(first_name=first_name,
                                    last_name=last_name,company_name=company_name,
